 ...
 */
export const make_outline = async (sId: string, end: number, index: number): Promise<any> => {
    try {
        const response = await axios.get('https://dynamic-202-239.informatik.uni-bremen.de:5000/outline', {
            params: {
                sessionId: sId,
                end: end,
                index: index,
